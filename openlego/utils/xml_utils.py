...
def get_related_parameter_uid(elem_param, full_xml):
    ...
    if isinstance(elem_param.find('relatedParameterUID'), _Element):
        ...
        related_instance = get_element_of_uid(full_xml, elem_param.find('relatedInstanceUID').text)
...
def dictify_loopnesting(r):
    ...
    if r.tag == 'loopNesting' or r.tag == 'loopElements' or r.tag == 'functionElements':
        for x in r.iterchildren():
            basic_list.append(dictify_loopnesting(x))
        ...
        for x in r.iterchildren():
            basic_dict[d['relatedUID']].extend(dictify_loopnesting(x))
        return basic_dict
